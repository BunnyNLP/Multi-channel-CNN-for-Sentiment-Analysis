...
    比如lexicon(fieldname):得到某字段的全部分词。当然其中最重要的方法就是search方法。
        '''
        query = QueryParser("content", ix.schema).parse(key)#生成查询对象,类似于：myquery = And([Term("content", u"apple"), Term("content", "bear")])
        ...
        results = searcher.search(query,limit=1,filter=allow_q,terms=True)#以query对象为参数调用searcher的search方法.得到查询result.
